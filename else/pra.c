...
{   
    ...
    fout = fopen("prac_test_distance.txt", "w");
    fout2 = fopen("prac_test_predecessor.txt", "w");
    
    foutd = fopen("prac_floyd_warshall_distance.txt", "w");
    foutp = fopen("prac_floyd_warshall_predecessor.txt", "w");
        ...
        for(i=0; i<ver_num; i++)
        {
            ...
            {   
                for(l=0; l<=k; l++)
                {
                    ...
                    {
                        if((distance[i][l] != INF && distance[l][j] != INF))
                        {
                            fprintf(fout, "(%d, %d, %d)-(%d, %d, %d)  ", i, l, j, distance[i][j], distance[i][l], distance[l][j]);
                            fprintf(fout2, "(%d, %d, %d)-(%d, %d, %d)  ", i, l, j, distance[i][j], distance[i][l], distance[l][j]);
                ...
                for(l=0; l<=k; l++)
                {
                    if(distance[i][j] > (distance[i][l] + distance[l][j]))
                    {
                        if((distance[i][l] != INF && distance[l][j] != INF))
                        {
                            fprintf(fout, "%s (%d, %d, %d)-(%d, %d, %d)  ","negative cycle at", i, l, j, distance[i][j], distance[i][l], distance[l][j]);
                            fprintf(fout2, "%s (%d, %d, %d)-(%d, %d, %d)  ","negative cycle at", i, l, j, distance[i][j], distance[i][l], distance[l][j]);
                            fprintf(fout, "%s", "end");
                            fprintf(fout2, "%s", "end");
                            fclose(fout);
                            fclose(fout2);
